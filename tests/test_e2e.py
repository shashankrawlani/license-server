...
async def test_full_user_journey_e2e(client, session):
    """
    ...
    with patch("license_server.routes.send_email") as mock_send:
        ...
        assert "Verification email sent" in reg_resp.json()["message"]
        # Note: background_tasks.add_task might not be easily checkable here without more setup
    ...
    val_resp = await client.post(f"/validate-license?license_key={community_key}")
    ...
    val_ent_resp = await client.post(f"/validate-license?license_key={enterprise_key}")
    ...
    val_rev_resp = await client.post(f"/validate-license?license_key={enterprise_key}")
    assert val_rev_resp.status_code == 404

    # Step 9: List all licenses (Admin)
    list_resp = await client.get("/licenses", headers={"Authorization": f"Bearer {admin_key}"})
    assert list_resp.status_code == 200
    all_licenses = list_resp.json()
    assert len(all_licenses) >= 2

async def test_admin_unauthorized_access(client):
    """Verify admin endpoints are protected."""
    auth_header = {"Authorization": "Bearer invalid-key"}
